...
class ReportController extends AppBaseController
{
     ...
     * @return Response
     */
    public function kalengNuReport(Request $request)
    {
     ...
     * @return Response
     */
    public function jpzisnuReport(Request $request)
    {   
        $kecamatan = Kecamatan::select('id', 'name')
            ->whereParentId(0)
            ->whereIsActive(1)
            ...
            if($request->type == 'income') {
                foreach($ziswafCat as $item) {
                    ...
                    if($item->id == 7) {
                        $program = DB::table('donates')
                            ...
                            ->leftJoin('locations', 'locations.id', 'donates.location_id')
                            ->when(!empty($request->desa) || !empty($request->kecamatan), function($q) use($request) {
                                if(!empty($request->kecamatan) && !empty($request->desa)) {
                                    $q->where('locations.id', $request->desa);
                                } else {
                                    $q->where('locations.parent_id', $request->kecamatan);
                                }
                            })
                            ->where(DB::raw("DATE_FORMAT(donates.date_donate, '%m-%Y')"), $date)
                            ->whereNull('donates.deleted_at')
                ...
                foreach($outcomeCat as $item) {
                    $totalOutcome = Outcome::select('outcomes.category_id', DB::raw("SUM(outcomes.nominal) as total_donate"))
                        ...
                        ->where(DB::raw("DATE_FORMAT(outcomes.date_outcome, '%m-%Y')"), $date)
    ...
    public function exportJpzisnu(Request $request)
    {
        ...
        $from_date = isset($request->from) ? $request->from : 1;
        $to_date = isset($request->to) ? $request->to : 12;
    ...
    public function exportJpzisnuDetail($month, Request $request)
    {
        $kecamatans = Kecamatan::select('id', 'name', 'created_at')
            ->when(!empty($request->kecamatan), function($q) use($request) {
            ...
            ->orderBy('name', 'asc')
            ->get();
        $desas = Desa::select('id', 'parent_id', 'name', 'created_at')
            ->when(!empty($request->desa) || !empty($request->kecamatan), function($q) use($request) {
                if(isset($request->kecamatan) && isset($request->desa)) {
                    $q->where('locations.id', $request->desa);
                } elseif(isset($request->kecamatan)) {
        ...
        $kabupaten = $this->_processDetail([], $month);

        $kecamatan = [];
        foreach($kecamatans as $row) {
            $split = $this->_processDetail(['kecamatan' => $row->id], $month);
        ...
        foreach($desas as $row) {
            $split = $this->_processDetail(['desa' => $row->id], $month);
            array_push($desa, [
                'location' => $row->name,
                'data' => [
                    'result' => $split['result'],
        ...
        $filename = 'LAPORAN JPZISNU PENERIMAAN & PENGELUARAN BULAN ' . strtoupper(Carbon::parse($data['month'])->isoFormat('MMMM')) . ' TAHUN ' . $data['year'] . '.xlsx';
    ...
    private function _processDetail($request, $month)
    {
        $ziswafCat = Ziswaf::select('id', 'title', 'created_at')->orderBy('title', 'asc')->get();
        $outcomeCat = OutcomeCategory::select('id', 'name', 'created_at')->orderBy('name', 'asc')->get();
        ...
        foreach($ziswafCat as $item) {
            $totalIncome = Donate::select('donates.type_id', DB::raw("SUM(donates.total_donate) as total_donate"))
                ->leftJoin('locations', 'locations.id', 'donates.location_id')
                ->when(isset($request['kecamatan']), function($q) use($request) {
                    $q->where('locations.parent_id', $request['kecamatan']);
                })
                ->when(isset($request['desa']), function($q) use($request) {
                    $q->where('locations.id', $request['desa']);
                })
     ...
     * @return Response
     */
    public function midtransReport(Request $request)
    {
        $kecamatan = Kecamatan::select('id', 'name')
            ->whereParentId(0)
            ->whereIsActive(1)
        ...
        if($request->ajax()) {
            ...
            return DataTables::of($months)
                ->addColumn('action', 'admin.pages.reports.midtrans.datatables_actions')
                ->addIndexColumn()
                ->editColumn('income', '{{ "Rp " . number_format($income,0,",",".") }}')
                ->editColumn('outcome', '{{ "Rp " . number_format($outcome,0,",",".") }}')
                ->make(true);
    ...
    public function midtransReportShow(Request $request, $date)
    {
        ...
        return view('admin.pages.reports.midtrans.show')
            ->with('months', $months)
            ->with('desa', $desa)
            ->with('kecamatan', $kecamatan);
    }

    public function exportMidtrans(Request $request)
    {
        if($request->from > $request->to) {
            Session::flash('error', 'Bulan tidak valid');
            return redirect(route('admin.report.annual.index'));
